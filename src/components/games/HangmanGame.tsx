...
export default function HangmanGame({ celebrityId, onClose }: { celebrityId?: string; onClose: () => void }) {
  const [sessionId, setSessionId] = useState<string | null>(null);
  const [result, setResult] = useState<GameResult | null>(null);
  const addNotification = useNotifStore((state) => state.addNotification);

  const start = useMutation({
    mutationFn: () => startGameSession({ gameType: 'hangman', celebrityId }),
    onSuccess: (session: any) => setSessionId(session.sessionId),
  });
  const complete = useMutation({
    mutationFn: () => completeGameSession(sessionId!, { word: 'test', wrongLetters: [] }),
    ...
    return (
      <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
        <div className="bg-sw-card p-6 rounded-2xl">
          <button onClick={() => start.mutate()} className="btn-gold">Start Hangman</button>
