...
export default function TriviaGame({ celebrityId, onClose }: { celebrityId?: string; onClose: () => void }) {
  ...
  const complete = useMutation({
    mutationFn: () => {
      console.log('Completing trivia with answers:', answers);
      return completeGameSession(sessionId!, { answers });
    },
    ...
    onError: (err: any) => {
      ...
      addNotification({
        ...
        type: 'error',
        title: 'Error',
        body: err?.response?.data?.message || 'Failed to submit answers.',
        priority: 'normal',
        read: false,
        createdAt: new Date().toISOString(),
      });
