...
export default function WordScrambleGame({ celebrityId, onClose }: { celebrityId?: string; onClose: () => void }) {
  ...
  const complete = useMutation({
    mutationFn: () => completeGameSession(sessionId!, { word: 'example' }),
    ...
    return (
      <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
        <div className="bg-sw-card p-6 rounded-2xl">
          <button onClick={() => start.mutate()} className="btn-gold">Start Word Scramble</button>
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
      <button onClick={() => complete.mutate()} className="btn-gold">Submit Answer</button>
      <button onClick={onClose} className="btn-outline mt-2">Cancel</button>
    </div>
  );
}