...
// Q&A trivia — answers sent to server at end, server decides win/loss
      ...
      {Array.from({ length: total }).map((_, i) => (
        <div
          ...
          className={[
            ...
            i < current
              ? correct.includes(i) ? 'bg-win' : 'bg-loss'
              : i === current
              ? 'bg-gold scale-125'
              : 'bg-[#1E2440]',
          ].join(' ')}
...
export default function TriviaGame({ celebrityId, onResult }: TriviaGameProps) {
  ...
  const [answers, setAnswers] = useState<number[]>([])        // selected index per question
  const [correctAnswers, setCorrectAnswers] = useState<number[]>([]) // indices that were right (from review)
  ...
  // Socket: ticket game offer
  useEffect(() => {
    if (!socket) return
    const handler = (offer: TicketGameOffer) => { setTicketOffer(offer); setShowTicketOffer(true) }
  ...
  function handleAnswer(optionIndex: number) {
    ...
    // Show selected for 1.2s then advance
    setTimeout(() => {
  ...
  return (
    <div className="max-w-md mx-auto">
      ...
      <PrizeModal
        ...
        onClose={() => { setShowPrize(false); handlePlayAgain() }}
