...
function scramble(word: string): string {
  ...
  let attempts = 0
  do { result.sort(() => Math.random() - 0.5); attempts++ }
...
export default function WordScramble({ celebritySlug, onComplete }: WordScrambleProps) {
  ...
  const completeMutation = useMutation({
    mutationFn: (finalScore: number) =>
      completeGameSession(sessionId!, { answers: [finalScore] }),
  ...
  const advanceRound = useCallback(
    ...
    [roundIdx, rounds], // eslint-disable-line
  )
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(t => {
        if (t <= 1) { advanceRound(false); return SECONDS_PER_ROUND }
        return t - 1
      })
    }, 1000)
    return () => { if (timerRef.current) clearInterval(timerRef.current) }
  }, [starting, gameOver, roundIdx]) // eslint-disable-line
  ...
  return (
    <div className="space-y-5 max-w-sm mx-auto">
      <div className="flex items-center justify-between text-sm">
        <span className="text-gray-400">Round <span className="text-white font-bold">{roundIdx + 1}/{ROUND_COUNT}</span></span>
        <span className="text-gray-400">Score <span className="text-gold font-bold">{score}</span></span>
      </div>

      <div className="progress-track h-1">
        <motion.div
          className={cn('h-full rounded-full transition-colors', timeLeft > 10 ? 'bg-gold' : timeLeft > 5 ? 'bg-amber-400' : 'bg-loss')}
      ...
      {gameOver && !completeMutation.isPending && (
        <div className="card card-gold p-5 text-center space-y-1">
          <p className="font-heading font-bold text-xl text-white">{score}/{ROUND_COUNT} correct</p>
